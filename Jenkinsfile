pipeline {
  ...
  stages {
    stage('checkout') {
      steps {
        git branch: 'main', url: 'https://github.com/sukanth/JavaWebApp'
  ...
  // Test Java application
   ...
   // Build docker image in Jenkins
    ...
    stage('Build Docker Image') {
      ...
      steps {
        ...
        sh 'docker tag javawebapp sukanth/javawebapp:latest'
      }
    }
   ...
   // Push image to DockerHub registry
    ...
    stage('Push Image to dockerHUb') {
      steps {
        sh 'docker push sukanth/javawebapp:latest'
      }
      post {
        always {
          sh 'docker logout'
   ...
   // Pull docker image from DockerHub and run in EC2 instance 
    ...
    stage('Deploy Docker image to AWS instance') {
	  ...
	  sh "ssh -o StrictHostKeyChecking=no ${REMOTE_USER}@${REMOTE_SERVER} 'docker pull palakbhawsar/javawebapp'"
          sh "ssh -o StrictHostKeyChecking=no ${REMOTE_USER}@${REMOTE_SERVER} 'docker run --name javaApp -d -p 8081:8081 sukanth/javawebapp'"
          }
        }
      }
    }
